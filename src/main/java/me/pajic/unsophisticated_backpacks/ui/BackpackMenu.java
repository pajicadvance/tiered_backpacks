package me.pajic.unsophisticated_backpacks.ui;
...
public class BackpackMenu extends AbstractContainerMenu {
    ...
    public BackpackMenu(int containerId, Inventory playerInventory, ItemStack backpack) {
        super(Main.BACKPACK_MENU, containerId);
        this.backpack = backpack;
        if (backpack.is(ModItems.BACKPACKS)) {
            BackpackDimensions dimensions = Main.getBackpackDimensions(backpack);
    ...
    public void removed(@NotNull Player player) {
        ...
        if (container.items.isEmpty() || container.items.stream().allMatch(ItemStack::isEmpty)) {
            backpack.set(DataComponents.CONTAINER, ItemContainerContents.EMPTY);
        }
        else backpack.set(DataComponents.CONTAINER, ItemContainerContents.fromItems(container.items));
    }

    @Override
    public boolean stillValid(@NotNull Player player) {
        return backpack.is(ModItems.BACKPACKS);
    }
